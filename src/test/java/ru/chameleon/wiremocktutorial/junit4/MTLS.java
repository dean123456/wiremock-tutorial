...
public class MTLS {
    ...
    @Rule
    public WireMockRule mTlsWireMockRule = new WireMockRule(options().httpsPort(PORT1) // включение https
            .needClientAuth(true) // требование клиентских сертификатов
            .trustStorePath("src/test/resources/store/server/server-truststore.jks") // путь до хранилища с доверенными сертификатами
            .trustStorePassword("spassword") // пароль хранилища с доверенными сертификатами
            .keystorePath("src/test/resources/store/server/server-keystore.jks") // путь до хранилища ключей и сертификатов
            .keyManagerPassword("spassword") // пароль менеджера ключей
            .keystorePassword("spassword")); // пароль хранилища ключей

    @Test
    public void mTLSTest() throws NoSuchAlgorithmException, KeyStoreException, KeyManagementException, IOException, CertificateException, UnrecoverableKeyException {
        ...
        mTlsWireMockRule.stubFor(get(urlEqualTo(url))
